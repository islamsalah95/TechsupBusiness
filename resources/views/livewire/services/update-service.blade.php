<div class="container mt-5">
    ...
    <div class="col-md mb-4 mb-md-0">
        <div class="card">
            <div class="card-body">
                <form wire:submit.prevent="submit" class="browser-default-validation">
                    @csrf
                    ...
                    <!-- Status Dropdown -->
                    <div class="mb-3">
                        <label for="status" class="form-label">{{ __('services/edit.labels.status') }}</label>
                        <select 
                            id="status" 
                            class="form-select @error('status') is-invalid @enderror" 
                            wire:model="status"
                        >
                            <option value="" disabled>{{ __('services/edit.labels.status') }}</option>
                            <option value="1">{{ __('services/edit.labels.active') }}</option>
                            <option value="0">{{ __('services/edit.labels.inactive') }}</option>
                        </select>
                        @error('status') 
                            <div class="invalid-feedback">{{ $message }}</div> 
                        @enderror
