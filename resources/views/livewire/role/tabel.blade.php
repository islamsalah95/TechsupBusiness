        ...
        <table class="table table-striped" id="categoriesTable">
            ...
            <tbody>
                ...
                <tr>
                    <td>{{ $role->name ?? '' }}</td>
                    <td>
                        <form action="{{ route('roles.destroy', ['role' => $role->id, 'locale' => app()->getLocale()]) }}" method="POST" style="display: flex; align-items: center; gap: 0.5rem;">
                            <a class="btn btn-info" href="{{ route('roles.show', ['role' => $role->id, 'locale' => app()->getLocale()]) }}">@lang('services/index.show')</a>
                            @can('role-edit')
                                <a class="btn btn-primary" href="{{ route('roles.edit', ['role' => $role->id, 'locale' => app()->getLocale()]) }}">@lang('services/index.edit')</a>
                            @endcan
                            @csrf
                            @method('DELETE')
                            @can('role-delete')
